...
class CyberBattleEnv(CyberBattleSpaceKind):
    ...
    # Der Beobachtungsgraph wird bei jedem Schritt neu aufgebaut (Teilgraph
    # plus Deepcopy) und kostete im Profil 14,2 Prozent der Laufzeit. Gelesen
    # wird er nur von graph_wrapper.py; die MARLon-Wrapper entfernen den
    # Schluessel wieder, ohne ihn je anzufassen. Wer ihn nicht braucht, setzt
    # build_explored_network = False und bekommt einen leeren Graphen.
    build_explored_network: bool = True
    ...
    def __get_explored_network(self) -> networkx.DiGraph:
        ...
        Liefert einen leeren Graphen, wenn build_explored_network aus ist.
        """
        if not self.build_explored_network:
