...
class DefenderAgentActions:
    ...
    def on_attacker_step_taken(self):
        ...
        for node_id, node_info in self._environment.nodes():
            ...
            # Ausdruecklich gesperrte Ports gelten als nicht verfuegbar, genau
            # wie gestoppte Dienste. Siehe Kommentar am Kopf dieser Aenderung.
            gesperrte_ports = {
                regel.port
                for regel in node_info.firewall.incoming
                if regel.permission == model.RulePermission.BLOCK
            }
            for service in node_info.services:
                total_service_weights += service.sla_weight
                erreichbar = service.running and service.name not in gesperrte_ports
                running_service_weights += service.sla_weight * int(erreichbar)
